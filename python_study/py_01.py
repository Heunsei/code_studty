...
# 딕셔너리로 이루어진 리스트에서 값들을 하나하나 꺼내서 저장
...
# 받아온 user 리스트에 있는 첫 딕셔너리부터 하나하나 key에 접근해서 tmp에 새로운 딕셔너리 제작 후 검사
    ...
    print(f'잘못된 데이터로 구성된 유저의 수는 {global_false} 입니다.') 
    pprint.pprint(user_list) 
      
def is_validation(tmp) :
    global global_false
    fail_count = 0  
    fail_list = [] 
    is_blocked = False
# if문은 걍 노가다 돌리자
    ...
    if len(tmp['name']) < 2 or len(tmp['name']) > 30 :
        fail_count += 1
        fail_list.append('name')
        tmp['name'] = None
        
    if len(tmp['website']) == 0 :
        fail_count += 1
        fail_list.append('website')
        tmp['website'] = None
